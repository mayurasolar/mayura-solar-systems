    ...
    <>
      ...
      <NavbarContent className="sm:hidden" justify="start">
        ...
        >
          ...
          <span className={`block w-6 h-0.5 bg-gray-900 transition-all duration-300 ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}></span>
          <span className={`block w-6 h-0.5 bg-gray-900 transition-all duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
        </button>
      </NavbarContent>
        ...
        <NavbarBrand>
          <Link href="/" className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-[10px] text-gray-500 font-medium">Solar Systems</p>
            </div>
          </Link>
        </NavbarBrand>
      </NavbarContent>
      ...
      {isMenuOpen && (
        <div className="fixed top-20 left-0 right-0 bg-white shadow-lg z-[100] sm:hidden border-t-2 border-gray-200">
          <div className="flex flex-col p-6 space-y-4">
            {menuItems.map((item) => (
              <a
                key={item.href}
